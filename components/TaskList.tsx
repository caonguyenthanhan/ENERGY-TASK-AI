    ...
    return (
      <div style={{ ...style, paddingBottom: '12px' }} {...ariaAttributes}>
        <div
          ...
          className={`relative cursor-pointer p-4 rounded-2xl border transition-all hover:border-indigo-500/50 hover:bg-zinc-800/80 h-full flex flex-col justify-between ${
    ...
    <div className="w-full lg:w-[400px] xl:w-[450px] flex flex-col bg-zinc-900/40 border border-zinc-800 rounded-3xl p-6 h-[calc(100vh-10rem)] sticky top-8">
      ...
      <div className="flex-1 overflow-hidden" ref={ref}>
        {filteredTasks.length === 0 ? (
          ...
          <List
            defaultHeight={bounds.height || 400}
            rowCount={filteredTasks.length}
            rowHeight={120}
            style={{ width: '100%', height: bounds.height || 400 }}
